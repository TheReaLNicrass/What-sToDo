...
async function login(client, email, password = "demo1234") {
  ...
  return {
    auth: { Authorization: `Bearer ${result.body.token}` },
    body: result.body,
  };
}
...
test("kompletter CRUD-Flow für Projekte, Mitglieder, Tasks und Assignees funktioniert", async () => {
  ...
  try {
    ...
    const members = await client.request(`/api/projects/${projectId}/members`, {
      headers: auth,
    });
    assert.equal(members.body.members.length, 2);

    const promote = await client.request(
      `/api/projects/${projectId}/members/${lisa.id}`,
      {
        method: "PUT",
        headers: auth,
    ...
    assert.equal(
      promote.body.project.members.find((member) => member.user.id === lisa.id)
        .role,
      "admin",
    );
    ...
    const taskDetails = await client.request(`/api/tasks/${taskId}`, {
      headers: auth,
    });
    ...
    const subtasks = await client.request(`/api/tasks/${taskId}/subtasks`, {
      headers: auth,
    });
    assert.equal(subtasks.body.tasks.length, 1);

    const employeeId = users.body.users.find(
    ...
    const assignees = await client.request(`/api/tasks/${taskId}/assignees`, {
      headers: auth,
    });
    ...
    const deleteAssignee = await client.request(
      `/api/tasks/${taskId}/assignees/${lisa.id}`,
      {
        method: "DELETE",
        headers: auth,
      },
